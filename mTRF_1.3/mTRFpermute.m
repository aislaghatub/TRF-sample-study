...
for ii = 1:nlambda
    for jj = 1:ntrials       
        ...
        intrial=((ii-1)*ntrials+jj)/(ntrials*nlambda);
        acctime=toc;
        tleft2=((acctime/intrial)*(1-intrial)/60)/60;
        disp([num2str(intrial*100,'%2.2f') '% ' num2str(((ii-1)*ntrials+jj)) '/' num2str(ntrials*nlambda) ' T-left(hrs): ' num2str(tleft2,'%2.2f') ])
    end
end
...
%             end
            ...
            for jj = 1:ntrials
             ...
             %   [pred_temp(ii,jj,:,:),r_temp(ii,jj,:,:),~,~] = mTRFpredict(stim{jj},resp{jj},current_model,fs,map,tmin,tmax,current_c);
                [~,r_perm(pp,ii,jj),~,~] = mTRFpredict(stim{jj},resp{jj},current_model,fs,map,tmin,tmax,current_c);
            end
        end
        
        acctime=toc;
        intrial=((pp-1)*nlambda+ii)/(iter*nlambda);
        tleft2=((acctime/intrial)*(1-intrial)/60)/60;
        disp([num2str(intrial*100,'%2.2f') '% ' num2str(pp) '/' num2str(iter) ' T-left(hrs): ' num2str(tleft2,'%2.2f') ])
